...
class Widget02 extends Component {
  render() {
    const { peakThread, daemonThread, className, cssModule, header, mainText, icon, color, value, footer, link, children, variant, ...attributes } = this.props;
    ...
    return (
      <Card>
        <CardBody className={card.classes} {...attributes}>
          ...
          <br />
          <div><strong>Peak Thread : {peakThread}</strong></div>
          <div><strong>Daemon Thread : {daemonThread}</strong></div>
        </CardBody>
        {cardFooter()}
      </Card>
    );
